Milestone1 (Regrade Attempt) Changes: 
    ERD Related changes And Entity
        - Removed Entities that no longer were included/needed: ATMPlacement, Card, Money. 
        - Linked Location directly to ATM.  
        - New Entities added; PhoneAssistance, KeyRing

    Non-ERD Related changes
        - Section VI: Resolved issue with Person Entity, with the issue of not being able to log in with an PIN, Phone, Card. Made Account directly related to a card that has to exist and card has foreign key for Person that must exist. 
        - Section IV: Updated the list of attributes and keys for each entity as well as their names to represent the current EER diagram. Each entity now has more than 2 attributes unlike before. 
